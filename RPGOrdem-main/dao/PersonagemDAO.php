...
class PersonagemDAO {
    ...
    public function listar() {
        $sql = "SELECT p.*, c.nome AS classe, o.nome AS origem 
                FROM personagem p 
                JOIN classe c ON p.classe_id = c.id 
                JOIN origem o ON p.origem_id = o.id 
                ORDER BY p.nome";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute();

        //TODO - Chamar o MAP para converter o retorno do banco (array_associativo) para objeto
